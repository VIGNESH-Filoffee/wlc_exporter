...
import (
	...
	"github.com/yankiwi/wlc_exporter/util"
...
// ParseVersion parses cli output and tries to find the model number of the running OS
func (c *systemCollector) ParseModel(ostype string, output string) (SystemModel, error) {
	log.Debugf("OS: %s\n", ostype)
	log.Debugf("output: %s\n", output)
	// if ostype != rpc.ArubaInstant && ostype != rpc.ArubaController && ostype != rpc.ArubaSwitch && ostype != rpc.ArubaCXSwitch {
	// 	return SystemModel{}, errors.New("'show version' is not implemented for " + ostype)
	// }.
	...
	modelRegexp := make(map[string]*regexp.Regexp)
	// modelRegexp[rpc.ArubaInstant], _ = regexp.Compile(`MODEL:\s*(\d+)`)
	modelRegexp[rpc.ArubaController], _ = regexp.Compile(`MODEL:\s*(\d+)`)
	// modelRegexp[rpc.ArubaSwitch], _ = regexp.Compile(`System Description\s*:\s*(.*?)(?:,\s*revision|\s*$)`)
	// modelRegexp[rpc.ArubaCXSwitch], _ = regexp.Compile(`Product Name\s*:\s*([^\n]+)`)/

	lines := strings.Split(output, "\n")
	for _, line := range lines {
		matches := modelRegexp[ostype].FindStringSubmatch(line)
		if matches == nil {
			continue
		}
		return SystemModel{Model: matches[1]}, nil
	}
	return SystemModel{}, errors.New("model string not found")
}
...
// ParseVersion parses cli output and tries to find the model number of the running OS
func (c *systemCollector) ParseClients(ostype string, output string) (SystemModel, error) {
	log.Debugf("OS: %s\n", ostype)
	log.Debugf("output: %s\n", output)
	if ostype != rpc.ArubaController {
		return SystemModel{}, errors.New("'show version' is not implemented for " + ostype)
