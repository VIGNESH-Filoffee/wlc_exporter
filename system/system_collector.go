...
func init() {
	l := []string{"target", "serial"}
	versionDesc = prometheus.NewDesc(prefix+"version", "Running OS version", append(l, "version"), nil)

	// Added By SG
	modelDesc = prometheus.NewDesc(prefix+"model", "Running OS version", append(l, "model"), nil)
...
// CollectVersion collects version informations from Aruba Devices
func (c *systemCollector) CollectModel(client *rpc.Client, ch chan<- prometheus.Metric, labelValues []string) error {
	...
	// case "ArubaSwitch":
	// 	out, err = client.RunCommand([]string{"show lldp info local"})
	// 	if err != nil {
	// 		return err
	// 	}
	// case "ArubaCXSwitch":
	...
	item, err := c.ParseModel(client.OSType, out)
	if err != nil {
		return err
	}
	l := append(labelValues, item.Model)
	ch <- prometheus.MustNewConstMetric(modelDesc, prometheus.GaugeValue, 1, l...)
	return nil
}
...
func (c *systemCollector) Collect(client *rpc.Client, ch chan<- prometheus.Metric, labelValues []string) error {
	...
	err := c.CollectModel(client, ch, labelValues)
	if err != nil {
		log.Debugf("CollectModel for %s: %s\n", labelValues[0], err.Error())
	}
